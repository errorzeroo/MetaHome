...
if __name__ == "__main__":
    # URL 및 경로 설정
    url = "https://data.seoul.go.kr/dataList/OA-15067/S/1/datasetView.do?utm_source=chatgpt.com"
    download_folder = r"C:\\Users\\user\\Downloads"  # 브라우저 기본 다운로드 경로
    destination_folder = r"C:\\Lecture\\final"  # 최종 저장할 폴더
    target_file_name = "bus.csv"
...
# 데이터 불러오기
df = pd.read_csv('C:/Lecture/final/bus.csv', encoding='cp949')  # 파일 경로와 인코딩 확인

# 필요한 열만 선택
filtered_df = df[['정류소명', 'Y좌표', 'X좌표']]

# 중복 제거
filtered_df = filtered_df.drop_duplicates()

# 위도와 경도 중 하나라도 값이 없는 행 제거
filtered_df = filtered_df.dropna(subset=['Y좌표', 'X좌표'])

# 결과를 CSV로 저장
filtered_df.to_csv('C:/Lecture/final/버스_전처리_최종.csv', index=False, encoding='utf-8-sig')

import pandas as pd
import cx_Oracle

# CSV 파일 로드
file_path = 'C:/Lecture/final/버스_전처리_최종.csv'
